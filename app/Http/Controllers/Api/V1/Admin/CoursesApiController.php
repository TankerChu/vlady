...
class CoursesApiController extends Controller
{
    ...
    public function store(StoreCourseRequest $request)
    {
        ...
        if ($request->input('courses_image', false)) {
            $course->addMedia(storage_path('tmp/uploads/' . $request->input('courses_image')))->toMediaCollection('courses_image');
        }
    ...
    public function update(UpdateCourseRequest $request, Course $course)
    {
        ...
        if ($request->input('courses_image', false)) {
            if (!$course->courses_image || $request->input('courses_image') !== $course->courses_image->file_name) {
                $course->addMedia(storage_path('tmp/uploads/' . $request->input('courses_image')))->toMediaCollection('courses_image');
            }
        } elseif ($course->courses_image) {
            $course->courses_image->delete();
        }
